...
for i in range(NO_POLICIES):
    ...
    model.append(PPO("MlpPolicy", env, verbose=0, tensorboard_log="./logs/{}/".format(SAVE_ID)))
counter = 0
#trainings loop
for j in range(int(NO_TOTAL_STEPS/AVERAGE_EVERY)):
    ...
    env = gym.make(GYM_NAME)
    env = Monitor(env,"./logs/{}".format(SAVE_ID))
    model = [PPO.load("./models/{}/model_{}".format(SAVE_ID,str(j*NO_POLICIES+1+i).zfill(3))) for i in range(NO_POLICIES)]
    #average models
    env = gym.make(GYM_NAME)
    env = Monitor(env,"./logs/{}".format(SAVE_ID))
